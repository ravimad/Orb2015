...
object ConcTrees {
  ...
  abstract class RunContext {
    ...
    def inputFromPoint(i: Int) = {
      ...
      for (i <- 0 until len) {
        contree = inserttime(contree, BigInt(0), BigInt(0))._1
      }
      contree
    }
  ...
  object ConcatContext extends RunContext {
    override def coeffs = scalaList[BigInt](8, 29)
    override val filePrefix = "conq-concat" // the abbrevation used in the paper
    override val concreteInstFun = (contree: Conc[BigInt]) => concatNonEmptytime(contree, Single[BigInt](BigInt(0)))._2
  }
