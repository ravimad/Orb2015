...
import stepsAnalysis.ConcTrees._
...
object Conqueue {
  ...
  def emptyNum[T] = {
    Conqueue3[T](Val1(Tip1(Empty[T]())), Nil())
  }
  ...
  abstract class RunContext {
    ...
    def inputFromPoint(i: Int) = {
      ...
      for (i <- 0 until len) {
        conq = pushLeftAndPaytime(Single[BigInt](BigInt(0)), conq)._1
      }
      conq
    }
  ...
  object PushContext extends RunContext {
    override def coeffs = scalaList[BigInt](124)
    override val filePrefix = "conq-push" // the abbrevation used in the paper
    override val concreteInstFun = (conq: Conqueue3[BigInt]) => pushLeftAndPaytime(Single[BigInt](BigInt(0)), conq)._2
  }
