...
"""
  ...
  def plot(testSize: scalaListBuffer[BigInt], ops: List[BigInt], orb: List[BigInt], function: String, orbOrInst: String) {
  ...
  def dumplogdata(testSize: scalaList[Int], ops: List[BigInt], orb: List[BigInt], function: String, orbOrInst: String) {
    ...
    var j = 0
    for (i <- testSize) {
      orbout.write(s"${scala.math.log(i.doubleValue) / scala.math.log(2)} ${orb(j)}\n")
      j = j + 1
    }
    orbout.close()
  ...
  def dumpdirdata(testSize: scalaList[Int], ops: scalaListBuffer[BigInt], orb: scalaListBuffer[BigInt], function: String, orbOrInst: String, dirname: String) {
    val dir = new java.io.File(s"results/${dirname}/")
    dir.mkdirs
    val file = new File(dir, s"${orbOrInst}${function}.data")
    val orbstream = new FileWriter(file)
    val orbout = new BufferedWriter(orbstream)
    var j = 0
    for (i <- testSize) {
  ...
  def goesThrough(ops: scalaListBuffer[BigInt], model: scalaListBuffer[BigInt], subsval: scalaList[scalaListBuffer[BigInt]]): (Boolean, Int) = {
    var j = 0
    while (j != ops.size) {
      var x = ops(j)
      var i = 0
  ...
  def minreport(ops: scalaListBuffer[BigInt], model: scalaListBuffer[BigInt], subsval: scalaList[scalaListBuffer[BigInt]], points: scalaListBuffer[BigInt], here: Int): (scalaListBuffer[BigInt], BigInt) = {
  ...
  def minresults(ops: scalaListBuffer[BigInt], model: scalaListBuffer[BigInt], id: List[String], subsval: scalaList[scalaListBuffer[BigInt]], points: scalaListBuffer[BigInt], filename: String) {
  ...
  def mindirresults(ops: scalaListBuffer[BigInt], model: scalaListBuffer[BigInt], id: List[String],
                    subsval: scalaList[scalaListBuffer[BigInt]], points: scalaListBuffer[BigInt], filename: String,
