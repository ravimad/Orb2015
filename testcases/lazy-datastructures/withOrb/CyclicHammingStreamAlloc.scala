 ...
 */
object MergeAndHammingNumbersAlloc {
  /**
  ...
  } ensuring (_ => time <= ? && alloc <= ?)  // Orb result: 11
  ...
  def next(f: SCons, s: SCons): SCons = {
    ...
    s.tail
  } ensuring(_ => time <= ? && alloc <= ?) // Orb result: time <= 250
  ...
  } ensuring(_ => time <= ? * n + ?  && alloc <= ? * n + ?) // Orb result: 261 * n - 260
  ...
  } ensuring(_ => time <= ? && alloc <= ?) // Orb result: 63
